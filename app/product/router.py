...
from ..users.auth import get_current_user_id, JWTBearer
...
@router.post('/product/create', dependencies=[Depends(JWTBearer())])
...
@router.post('/product/{product_id}', dependencies=[Depends(JWTBearer())])
...
@router.get('/cart', dependencies=[Depends(JWTBearer())])
...
@router.post('/cart/add_item', dependencies=[Depends(JWTBearer())])
...
@router.delete('/cart/delete_item', dependencies=[Depends(JWTBearer())])
async def add_item_to_cart(session: SessionDep, product_id: int, user_id: int = Depends(get_current_user_id)):
    cart = await ss.get_or_create_cart(session=session, user_id=user_id)
...
@router.post('/cart/delete_all_item', dependencies=[Depends(JWTBearer())])
